...
private val opcodes = mapOf(
        ...
        "gtir" to { inp: Registers, op: Registers -> inp[op.c] = if (op.a > inp[op.b]) 1 else 0 },
        "gtri" to { inp: Registers, op: Registers -> inp[op.c] = if (inp[op.a] > op.b) 1 else 0 },
        "gtrr" to { inp: Registers, op: Registers -> inp[op.c] = if (inp[op.a] > inp[op.b]) 1 else 0 },
        "eqir" to { inp: Registers, op: Registers -> inp[op.c] = if (op.a == inp[op.b]) 1 else 0 },
        "eqri" to { inp: Registers, op: Registers -> inp[op.c] = if (inp[op.a] == op.b) 1 else 0 },
        "eqrr" to { inp: Registers, op: Registers -> inp[op.c] = if (inp[op.a] == inp[op.b]) 1 else 0 })

object Day16 {

    private val file = File("src/main/kotlin/y2018/day16/sample.txt")
