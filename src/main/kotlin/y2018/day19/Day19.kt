...
object Day19 {
    ...
    val opcodes = mapOf(
        ...
        "gtir" to { inp: IntArray, op: Instruction -> inp[op.c] = if (op.a > inp[op.b]) 1 else 0 },
        "gtri" to { inp: IntArray, op: Instruction -> inp[op.c] = if (inp[op.a] > op.b) 1 else 0 },
        "gtrr" to { inp: IntArray, op: Instruction -> inp[op.c] = if (inp[op.a] > inp[op.b]) 1 else 0 },
        "eqir" to { inp: IntArray, op: Instruction -> inp[op.c] = if (op.a == inp[op.b]) 1 else 0 },
        "eqri" to { inp: IntArray, op: Instruction -> inp[op.c] = if (inp[op.a] == op.b) 1 else 0 },
        "eqrr" to { inp: IntArray, op: Instruction -> inp[op.c] = if (inp[op.a] == inp[op.b]) 1 else 0 }
    )
