 ...
 */
fun <T> zip(vararg lists: List<T>): List<List<T>> = zip(*lists, transform = { it })
...
inline fun <T, V> zip(vararg lists: List<T>, transform: (List<T>) -> V): List<V> {
    ...
    val iterators = lists.map(List<T>::iterator)
    var i = 0
    while (i < minSize) {
        list.add(transform(iterators.map(Iterator<T>::next)))
        i++
    }
    return list
}
